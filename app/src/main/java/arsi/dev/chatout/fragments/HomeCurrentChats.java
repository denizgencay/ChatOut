...
public class HomeCurrentChats extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_home_current_chats, container, false);
        ...
        recyclerView = view.findViewById(R.id.current_chats_recycler_view);
        ...
        documentReference.addSnapshotListener(new EventListener<DocumentSnapshot>() {
            ...
            public void onEvent(@Nullable DocumentSnapshot documentSnapshot, @Nullable FirebaseFirestoreException e) {
                if (documentSnapshot.get("blockedArray") != null)
                    ...
                    blockedUsers = new ArrayList<>();
                if (documentSnapshot.get("blockedByArray") != null)
                        ...
                        if (queryDocumentSnapshots != null) {
                            for (int i = 0; i < queryDocumentSnapshots.getDocuments().size(); i++) {
                                ...
                                if (snapshot.getId().equals(activeChat1)) {
                                    ...
                                    if (!blockedUsers.contains(snapshot.getString("creatorUid"))
                                            && !blockedByUsers.contains(snapshot.getString("creatorUid")) && !snapshot.getId().equals(activeChat1)) {
                                        String documentId = snapshot.getId();
                                        Map<String, Object> data = snapshot.getData();
                                        String title = (String) data.get("title");
                                        String personNumber = (String) data.get("numberOfPerson");
